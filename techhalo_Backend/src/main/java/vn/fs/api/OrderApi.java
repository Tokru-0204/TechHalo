...
public class OrderApi {
    ...
    public ResponseEntity<Order> checkout(@PathVariable("email") String email, @RequestBody Cart cart) {
        ...
        amount = amount - cart.getDiscountCart();
        Order order = orderRepository.save(new Order(0L, new Date(), amount, cart.getDiscountCart(), cart.getAddress(), cart.getPhone(), 0, 0, "",
...
//		cartDetailRepository.deleteByCart(cart);
    ...
    @PostMapping("checkoutcardpayment/{email}")
    public ResponseEntity<Order> checkoutcardpayment(@PathVariable("email") String email, @RequestBody Cart cart) {
        if (!userRepository.existsByEmail(email)) {
            return ResponseEntity.notFound().build();
        }
        if (!cartRepository.existsById(cart.getCartId())) {
        ...
        amount = amount - cart.getDiscountCart();
        Order order = orderRepository.save(new Order(0L, new Date(), amount, cart.getDiscountCart(), cart.getAddress(), cart.getPhone(), 0, 1, "",
                userRepository.findByEmail(email).get()));
        for (CartDetail i : items) {
            OrderDetail orderDetail = new OrderDetail(0L, i.getQuantity(), i.getPrice(), i.getProduct(), order);
            orderDetailRepository.save(orderDetail);
...
//		cartDetailRepository.deleteByCart(cart);
    ...
    //confirmOrder, cancel, deliver, confirm, delivered
    @GetMapping("confirmOrder/{orderId}")
    public ResponseEntity<Void> confirmOrder(@PathVariable("orderId") Long id) {
        if (!orderRepository.existsById(id)) {
            return ResponseEntity.notFound().build();
        }
    ...
    public ResponseEntity<Void> confirm(@PathVariable("orderId") Long id) {
        if (!orderRepository.existsById(id)) {
            return ResponseEntity.notFound().build();
        }
        Order order = orderRepository.findById(id).get();
    ...
    public ResponseEntity<Void> delivered(@PathVariable("orderId") Long id) {
        if (!orderRepository.existsById(id)) {
            return ResponseEntity.notFound().build();
        }
        Order order = orderRepository.findById(id).get();
